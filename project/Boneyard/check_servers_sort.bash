...
for ((i = 1; i <= 35; i+=1))do
  ...
  status[$i]=$((echo 'echo "servers on $(hostname -i): $(ps -eLf | grep -v "grep" | grep -c "server_leaf")"' ; exit) | ssh -T -p 215 aus213l$i) 2>/dev/null&
  #status[i]="$((echo 'echo "servers on $(hostname -i): $(ps -eLf | grep -v "grep" | grep -c "server_leaf")"' ; exit) | ssh -T -p 215 aus213l$i)"
  PIDS[$i]=$!
  #echo ""
done

echo "BROKE!!!!!! Use check_servers_pretty.bash!"
