...
public:
   ...
   Matrix<T> mult(const Matrix<T>& matrixB, Matrix<T>& result, string computers[], int numComputers, string port) const
   {
      ...
      if (mSize > 1)
      {
         ...
         //{
            int i = 0;
            t[1] = thread(&Matrix<T>::runParallel, (a00 + a11), (b00 + b11), std::ref(m1), computers[i++], port);
            t[2] = thread(&Matrix<T>::runParallel, (a10 + a11), (b00)      , std::ref(m2), computers[i++], port);
            t[3] = thread(&Matrix<T>::runParallel,  a00       , (b01 - b11), std::ref(m3), computers[i++], port);
            t[4] = thread(&Matrix<T>::runParallel,  a11       , (b10 - b00), std::ref(m4), computers[i++], port);
            t[5] = thread(&Matrix<T>::runParallel, (a00 + a01), (b11)      , std::ref(m5), computers[i++], port);
            t[6] = thread(&Matrix<T>::runParallel, (a10 - a00), (b00 + b01), std::ref(m6), computers[i++], port);
            t[7] = thread(&Matrix<T>::runParallel, (a01 - a11), (b10 + b11), std::ref(m7), computers[i++], port);
