...
public:
   ...
   Matrix<T> mult(const Matrix<T>& matrixB, Matrix<T>& result, string computers[], int numComputers, string port) const
   {
      ...
      if (mSize > 1)
      {
         ...
         {
            t[1] = thread(&Matrix<T>::mult, (a00 + a11), (b00 + b11), std::ref(m1), computers, numComputers, port);
            t[2] = thread(&Matrix<T>::mult, (a10 + a11), (b00)      , std::ref(m2), computers, numComputers, port);
            t[3] = thread(&Matrix<T>::mult,  a00       , (b01 - b11), std::ref(m3), computers, numComputers, port);
            t[4] = thread(&Matrix<T>::mult,  a11       , (b10 - b00), std::ref(m4), computers, numComputers, port);
            t[5] = thread(&Matrix<T>::mult, (a00 + a01), (b11)      , std::ref(m5), computers, numComputers, port);
            t[6] = thread(&Matrix<T>::mult, (a10 - a00), (b00 + b01), std::ref(m6), computers, numComputers, port);
            t[7] = thread(&Matrix<T>::mult, (a01 - a11), (b10 + b11), std::ref(m7), computers, numComputers, port);
         }
         else
         {
            int i = 0;
   ...
   ***********************************************************************/
   bool readNet(Connection& net)
   {
      ...
      for (int i = 0; i < mSize; ++i)
      {
         //cerr << Pur << "Bytes Received: " << mSize * sizeof(T) << RCol << "\n";
   ...
   ***********************************************************************/
   bool writeNet(Connection& net) const
   {
      ...
      for (int i = 0; i < mSize; ++i)
      {
         //cerr << Pur << "Bytes Sent: " << mSize * sizeof(T) << RCol << "\n";
         if (!net.sendInt(this->mRows[i], mSize * (sizeof(T))))
         {
            //cerr << "Server closed connection\n";
            //cerr << "ERROR: " << net.strError << endl;
...
// Initialize the static variable for Matrix
template <class T>
