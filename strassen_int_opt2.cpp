...
public:
   ...
   Matrix<T> mult(const Matrix<T>& matrixB, Matrix<T>& result) const
   {
	 ...
	 // The only purpose of this is to modify the scope, and 
	 // have the matrixB quadrants be deleted because they are no longer needed.
	 {
         // Four quadrants for each matrix being multiplied
         Matrix<T> a00(getQuadrant(0, 0));
         Matrix<T> a01(getQuadrant(0, 1));
         ...
         {
            t1 = thread(&Matrix<T>::mult, (a00 + a11), (b00 + b11), std::ref(m1));
            t2 = thread(&Matrix<T>::mult, (a10 + a11), (b00)      , std::ref(m2));
            t3 = thread(&Matrix<T>::mult, (a00)      , (b01 - b11), std::ref(m3));
            t4 = thread(&Matrix<T>::mult, (a11)      , (b10 - b00), std::ref(m4));
            t5 = thread(&Matrix<T>::mult, (a00 + a01), (b11)      , std::ref(m5));
            t6 = thread(&Matrix<T>::mult, (a10 - a00), (b00 + b01), std::ref(m6));
            t7 = thread(&Matrix<T>::mult, (a01 - a11), (b10 + b11), std::ref(m7));
            //t1 = thread(&Matrix<T>::mult, (a00 + a11), (b00 + b11), std::ref(m1));
            //t2 = thread(&Matrix<T>::mult, (a10 + a11), (b00)      , std::ref(m2));
            //t3 = thread(&Matrix<T>::mult,  a00       , (b01 - b11), std::ref(m3));
            //t4 = thread(&Matrix<T>::mult,  a11       , (b10 - b00), std::ref(m4));
	 ...
	 // Try for some memory optimization...
	 if (mSize > thread_Stop)
	 {
            t1.join();
            t2.join();
	 ...
	 result = Matrix(m1 + m4 - m5 + m7, m3 + m5, m2 + m4, m1 + m3 - m2 + m6);
      }
      else
      {
