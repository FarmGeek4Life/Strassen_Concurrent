...
************************************************************************************/
void computeValue(int i, int j, promise<int> && result)
{
   int value = 0;
   C[i][j] = 0;
   for (int k = 0; k < K; ++k)
   ...
   //result.set_value_at_thread_exit(value);
}
...
int main()
{
   ...
   thread threadId[M * N];
   promise<int> results[M * N];
   future<int> values[M * N];
   int threadCount = 0;
   ...
   for (int i = 0; i < M; i++)
   {
      for (int j = 0; j < N; j++ )
      {
         values[threadCount] = results[threadCount].get_future();
         /* Now create the thread passing it tData as a parameter */
         threadId[threadCount++] = thread(computeValue, i, j, move(results[threadCount]));
      }
   }
